...
class ManagementAdmissionController extends Controller
{
    public function checkSchedule($inst_code)
    {
        $seat = ManagementSeat::where([
                ...
                if (sizeof($role_url_access_id) > 0) {
                    ...
                    if (in_array('self-admission-save', $url_data)) {
                        try {
                            ...
                            $seat->update([
                                'mgmt' => ($seat->m_mgmt != $seat->a_mgmt) ? $seat->mgmt - 1 : $seat->mgmt,
                                'a_mgmt' => ($seat->m_mgmt != $seat->a_mgmt) ? $seat->a_mgmt + 1 : $seat->a_mgmt
                            ]);

                            auditTrail($user_data->u_id, "Management admission successful for student id: {$created_student->s_id}");
                ...
                if (sizeof($role_url_access_id) > 0) {
                    ...
                    if (in_array('self-admission-save', $url_data)) {
                        try {
                            ...
                            auditTrail($user_data->u_id, "Management admission removed for student id: {$request->student_id}");
                ...
                if (sizeof($role_url_access_id) > 0) {
                    ...
                    if (in_array('self-admission-save', $url_data)) {
                        try {
                            ...
                            auditTrail($user_data->u_id, "Student Removed from Management Admission of student id: {$request->student_id}");
    ...
    public function pdf($appl_num)
    {
        try {
            $candidate = DB::table('pharmacy_management_register_student')
                ...
                ->where('seat_type', 'MANAGEMENT')
                ->first();

            $payment = PaymentTransaction::where('pmnt_stud_id', $candidate->student_id)
                ->where('pmnt_pay_type', 'MANAGEMENTADMISSIONFEES')
                ->where('trans_status', 'SUCCESS')
                ->first();

            if ($payment) {
                $pdf = Pdf::loadView('exports.management-admission', [
                    'students' => $candidate,
                    'payment' => $payment
                ]);
                return $pdf->setPaper('a4', 'portrait')
                    ->setOption(['defaultFont' => 'sans-serif'])
                    ->stream('management-admission-students.pdf');
            } else {
                return response()->json([
                    'error' =>  true,
